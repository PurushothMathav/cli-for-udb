...
class KissKhClient(BaseClient):
    ...
    def __init__(self, config, session=None):
        ...
        self.api_base = config.get('api_base', 'https://api.kisskh.io')  # Added for drama ID lookup
    ...
    def show_episode_results(self, items, *predefined_range):
        ...
        '''
        show_range = str(predefined_range[1]) if predefined_range and predefined_range[1] is not None else ""
        start, end = self._get_episode_range_to_show(items[0].get('episode'), items[-1].get('episode'), show_range, threshold=24)
    ...
    def fetch_drama_by_id(self, drama_id):
        try:
            url = f"{self.series_url}{drama_id}"
            ...
            if response.status_code == 200:
                return response.json()  # ✅ Return whole object
            else:
                print(f"[ERROR] Drama ID fetch failed.")
                return None
        except Exception as e:
            print(f"[ERROR] Exception while fetching drama by ID: {e}")
            return None
